...
import { CreditCard, DollarSign, CheckCircle2, Clock, RefreshCw, X, ShieldCheck, Coins, History } from 'lucide-react';
...
export const PaymentHistoryPage = () => {
    ...
    // Load Transactions
    const { data: txnData, isLoading: txnLoading, error: txnError, refetch: refetchTxns } = useTransactions({
        page,
        limit: 15,
        memberId: user?.role === 'ADMIN' ? undefined : user?.id
    });

    // Load Member Installments / Dues across all chits
    const {
        installments,
        loading: instLoading,
        error: instError,
    ...
    const pendingGatewayOrders = transactions.filter((t) => t.status === 'PENDING').length;
    ...
    const filteredInstallments = installments.filter((item) => {
        if (installmentFilterTab === 'ALL') return true;
        const currentStatus = item.paymentStatus || item.status || 'PENDING';
        return currentStatus === installmentFilterTab;
    });
        ...
        <div className="space-y-6 max-w-7xl mx-auto pb-12">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2 text-xs font-bold text-emerald-600 uppercase tracking-wider mb-1">
                        ...
                        <span>Payment & Financial Dues Center</span>
                    </div>
                    <h1 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">Payments & Dues</h1>
                    <p className="text-xs text-slate-500 mt-1">
                        Pay pending monthly installment dues, view receipts, and monitor verified transaction records.
                    </p>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                    <button
                        onClick={handleRefreshAll}
                        disabled={txnLoading || instLoading}
                        ...
                        title="Refresh Payments and Dues"
                    >
                        <RefreshCw className={`w-4 h-4 text-emerald-400 ${(txnLoading || instLoading) ? 'animate-spin' : ''}`} />
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white p-2 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-2">
                ...
                >
                    ...
                    <span>Pay Dues & Installments</span>
                    {pendingInstallments.length > 0 && (
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-emerald-600 text-white ml-1">
            ...
            {/* ══════════════════════════════════════════════════════════════ */}
            {/* TAB 1: PENDING DUES & PAY NOW                                */}
            {/* ══════════════════════════════════════════════════════════════ */}
            {mainTab === 'DUES' && (
                <div className="space-y-6">
                    {/* Total Pending Dues Banner */}
                    <div className="p-6 bg-slate-900 text-white rounded-3xl border border-slate-800 shadow-sm flex flex-wrap items-center justify-between gap-4">
                        <div>
                            <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-400 block">Total Pending Dues Across Circles</span>
                            <span className="text-2xl md:text-3xl font-black text-white mt-1 block">{formatCurrency(totalPendingDues)}</span>
                        </div>
                        <span className="px-4 py-2 bg-emerald-950 text-emerald-300 text-xs font-black rounded-xl border border-emerald-800">
                            {pendingInstallments.length} Pending Obligation{pendingInstallments.length !== 1 ? 's' : ''}
                        </span>
                    </div>
                            ...
                            <div className="lg:col-span-2 space-y-6">
                                {filteredInstallments.length === 0 ? (
                                    <EmptyState
                                        ...
                                        description={`No installments found matching status "${installmentFilterTab}".`}
                                    />
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {filteredInstallments.map((inst) => (
                                            <InstallmentCard key={inst._id} installment={inst} onPaymentSuccess={handleRefreshAll} />
                                        ))}
                                    </div>
                                )}
                            </div>
            ...
            {/* ══════════════════════════════════════════════════════════════ */}
            {/* TAB 2: PAYMENT HISTORY & TRANSACTIONS                        */}
            {/* ══════════════════════════════════════════════════════════════ */}
            {mainTab === 'HISTORY' && (
                <div className="space-y-6">
                    {/* Top Stat Cards */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <span className="text-xl font-black text-slate-900">{pendingGatewayOrders}</span>
                            </div>
                        </div>
